...
const Card = ({notHoverImage, hoverImage, bg, hoverBg, cardTitle, cardDescription, typePresentation}: CardProps) => {
    return (
        <Link
            to={`/presentations`}
            state={{ typePresentation }}
            className="h-full flex"
        >
            <button className={`w-full flex-1 transition-all md:hover:-translate-y-4 rounded-xl shadow-xl overflow-hidden`}>
                <div className={`group max-h-[150px] md:max-h-none h-full flex md:flex-col flex-grow`}>
                    <div className={`${bg} group-hover:${hoverBg} md:h-[40%] min-h-[150px] min-w-[40%] max-w-[80px] md:max-w-full md:w-auto flex justify-center items-center p-2 md:p-4`}>
...
>
    <button className={`w-full flex-1 transition-all md:hover:-translate-y-4 rounded-xl shadow-xl overflow-hidden`}>
        <div className={`group ${bg} hover:${hoverBg} max-h-[150px] md:max-h-none h-full flex md:flex-col flex-grow`}>
